package com.tyss.lms1.validation;
...
public class Validation {
	...
	public boolean validateId(int id) throws ValidationException{
		...
		boolean result = false;
		if(Pattern.matches("\\d{5}", String.valueOf(id))) {
			result = true; 
		} else {
			result = false;
	...
	public boolean validateBookId(int bookId) throws ValidationException {
		boolean result = false;
		if(Pattern.matches("\\d{4}", String.valueOf(bookId))) {
			result = true;
		} else {
			result = false;
	...
	public boolean validateCopies(int copies) throws ValidationException {
		boolean result = false;
		if(Pattern.matches("\\d{2}", String.valueOf(copies))) {
			result = true;
		} else {
			result = false;
